...
#   3.5 CSV-BASED ZT0 LOOKUP - build a filename -> zt_0 lookup from
#      MW_LD_DD_periods.csv's "Lights On" column (ground truth,
#      preferred over auto-detection whenever a file is covered by it)
...
# ============================================================
# 3.5 CSV-BASED ZT0 LOOKUP
#
# MW_LD_DD_periods.csv carries a "Lights On" column (e.g. "8am-8pm",
# "1am-1pm", "8pm-8am") that records each recording's actual light
# schedule per source_file. This is ground truth from the experiment
# itself, and should be preferred over derive_lights_on_from_data()'s
# auto-detection whenever a file is covered by it - auto-detection can
# be thrown off by sensor noise or averaged across multiple light
# transitions (see verify_light_column() for diagnosing that directly).
#
# Usage:
#   zt0_lookup <- build_zt0_lookup_from_periods(MW_LD_DD_periods)
#   run_batch_analysis(base_dir, group_folders, zt_0 = "csv", zt0_lookup = zt0_lookup)
# ------------------------------------------------------------

# ------------------------------------------------------------
# Parses a "Lights On" schedule label like "8am-8pm" / "1am-1pm" /
# "8pm-8am" into the ONSET (lights-on) clock time only, as "HH:00".
# Returns NA if the label is blank or doesn't match the expected
# "<hour><am|pm>-<hour><am|pm>" pattern.
# ------------------------------------------------------------
parse_lights_on_time <- function(label) {
  if (is.na(label) || label == "") return(NA_character_)
  onset <- str_trim(str_split_1(label, '-')[1])
  m <- str_match(tolower(onset), '^(\\d+)(am|pm)$')
  if (is.na(m[1, 1])) return(NA_character_)
  hour <- as.numeric(m[1, 2]); ampm <- m[1, 3]
...
# ------------------------------------------------------------
# Builds a named character vector: source_file (basename, matches the
# .txt filenames on disk) -> "HH:MM" lights-on time, from a
# MW_LD_DD_periods-style data.frame with 'source_file' and 'Lights.On'
# (or 'Lights On') columns.
#
# A file is only included if it resolves to exactly ONE distinct
# lights-on time across all its rows - files with conflicting labels
# are dropped from the lookup (with a warning) rather than guessed at,
# so callers fall back to "auto" for them instead of silently picking
# one of several disagreeing values.
# ------------------------------------------------------------
build_zt0_lookup_from_periods <- function(periods_df) {
  
  lights_on_col <- if ('Lights.On' %in% names(periods_df)) 'Lights.On' else if ('Lights On' %in% names(periods_df)) 'Lights On' else NA
  if (is.na(lights_on_col)) stop("periods_df has no 'Lights.On' / 'Lights On' column")
  
  df <- periods_df %>%
    mutate(.lights_on_raw = .data[[lights_on_col]],
           .zt0_time = sapply(.lights_on_raw, parse_lights_on_time)) %>%
    filter(!is.na(source_file), source_file != "", !is.na(.zt0_time))
  
  by_file <- df %>%
    group_by(source_file) %>%
    summarize(times = list(unique(.zt0_time)), .groups = "drop")
  
  conflicts <- by_file %>% filter(lengths(times) > 1)
  if (nrow(conflicts) > 0) {
    warning("Conflicting Lights On values found for: ", paste(conflicts$source_file, collapse = ", "),
            " - these files are excluded from the lookup, will fall back to auto-detection")
  }
  
  clean <- by_file %>% filter(lengths(times) == 1) %>% mutate(zt0 = sapply(times, `[[`, 1))
  setNames(clean$zt0, clean$source_file)
}
...
#
# zt_0 resolution order, per file:
#   1. If zt0_lookup is supplied AND basename(filename) is a name in
#      it -> use that CSV-sourced lights-on time (ground truth from
#      MW_LD_DD_periods.csv). This takes priority over "auto" even if
#      zt_0 = "auto" was passed, since a confirmed schedule beats an
#      inferred one whenever we have it.
#   2. Else if zt_0 == "auto" -> derive_lights_on_from_data() as before.
#   3. Else -> zt_0 used as-is (a fixed POSIXct passed by the caller).
# ------------------------------------------------------------
process_monitor_file <- function(filename,
  ...
  # ---- resolve zt_0: CSV lookup takes priority, then "auto", then as-is ----
  this_file_basename <- basename(filename)
  
  if (!is.null(zt0_lookup) && this_file_basename %in% names(zt0_lookup)) {
    csv_time <- zt0_lookup[[this_file_basename]]
    zt_0 <- ymd_hm(paste0('01-1-1 ', csv_time))
    flags <- c(flags, str_interp("zt_0 = ${csv_time} (from MW_LD_DD_periods.csv Lights On - ground truth, not auto-detected)"))
    
  } else if (identical(zt_0, "auto")) {
    ...
    if (!is.null(zt0_lookup)) {
      flags <- c(flags, "not found in MW_LD_DD_periods.csv Lights On lookup - fell back to auto-detection, worth verifying manually")
    }
  }
  # else: zt_0 was passed in as a fixed POSIXct by the caller - used as-is, no flag needed.
...
#
# zt0_lookup: optional named vector (basename(file) -> "HH:MM"), e.g.
# from build_zt0_lookup_from_periods(MW_LD_DD_periods). When supplied,
# any file whose basename is a name in this vector uses that CSV-sourced
# zt_0 instead of "auto"/the fixed zt_0 default - see process_monitor_file()
...
run_batch_analysis <- function(base_dir,
  ...
  for (group_name in group_folders) {
    ...
    for (f in txt_files) {
      ...
      result <- tryCatch(
        process_monitor_file(f, group_name, section2_start_day, dd_days, rasterplots_only, zt_0, zt0_lookup, output_root, sample_periods),
        error = function(e) {
